...
namespace MyBlog.src {
    public class DBDataProvider : IArticleDataProvider{
        ...
        public ArticleInfo GetArticleById(int id, bool getNext = false) {
            string sql = "SELECT TOP 1 [id], [title], [content] FROM [Articles] WHERE [id] <= @id ORDER BY [id] DESC";
            ...
            var row = dt.Rows[0];
            return new ArticleInfo(Convert.ToInt32(row["id"]), row["title"].ToString(), row["content"].ToString());
        }

        public ArticleInfo GetLatestArticle() {
            string sql = "SELECT TOP 1 [id], [title], [content] FROM [Articles] ORDER BY [id] DESC";
        ...
        public IEnumerable<ArticleInfo> GetArticleIdsAndTitles() {
            ...
            foreach(DataRow row in dt.Rows) {
                articles.Add(new ArticleInfo(Convert.ToInt32(row["id"]), row["title"].ToString(), ""));
            }
            return articles;
        }
        ...
        public int CreateArticle(string title, string content) {
            string sql = "INSERT INTO [Articles] ([title],[content]) OUTPUT Inserted.id VALUES(@title,@content)";
            var parameters = new Dictionary<string,object>();
            parameters.Add("@title", title);
            parameters.Add("@content", content);
        ...
        private int executeScalar (string sql, Dictionary<string,object>parameters) {
            using (var conn = new SqlConnection(_connectionString)) {
                conn.Open();
                using (var cmd = new SqlCommand(sql,conn)) {
                    if(parameters != null) {
